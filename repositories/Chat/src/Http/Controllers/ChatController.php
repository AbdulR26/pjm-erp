<?php
namespace Qollam\Chat\Http\Controllers;
...
{
    /**
     * Display the admin chat panel.
     */
    public function index()
    {
        $title = 'Chat Support';
        return view('chat-module::index', compact('title'));
    }
    ...
    public function sendMessage(Request $request, $customerId)
    {
        ...
        // Broadcast to Pusher channel (using existing app event)
        try {
            broadcast(new MessageSent($message))->toOthers();
        } catch (\Exception $e) {
            // Ignore broadcasting exceptions if pusher is offline locally
        }
